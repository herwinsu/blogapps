...
func PostsEditPost(c buffalo.Context) error {
	...
	return c.Redirect(302, "/posts/detail/%s", strconv.Itoa(post.ID))
}
...
func PostsDetail(c buffalo.Context) error {
	...
	author := &models.User{Username: post.AuthorID}
	if err := author.FindByUsername(tx); err != nil {
		return c.Error(404, err)
	}
	...
	if err := tx.BelongsToAs(post, "postid").All(&comments); err != nil {
		return errors.WithStack(err)
	}
	for i := 0; i < len(comments); i++ {
		u := models.User{Username: post.AuthorID}
		if err := u.FindByUsername(tx); err != nil {
			return c.Error(404, err)
		}
		comments[i].Author = u
	}
